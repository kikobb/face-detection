...
RUN apt-get update && apt-get install -y --no-install-recommends \
   apt-utils automake cmake cpio gcc g++ libatlas-base-dev libstdc++6 libtool \
   libusb-1.0.0-dev lsb-release make python3-pip python3-numpy python3-scipy libgtk-3-0 \
   pkg-config libavcodec-dev libavformat-dev libswscale-dev sudo udev unzip vim wget \
