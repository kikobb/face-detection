...
class CNN(object):
    ...
    def load_graph(self, model_filepath):
        ...
        # Define input tensor
        self.input = tf.placeholder(np.float32, shape=[None, 96, 96, 3], name='input')
    ...
    def test(self, data):
        ...
        output_tensor = self.graph.get_tensor_by_name("import/MobilenetV2/Predictions/Reshape_1:0")
        output = self.sess.run(output_tensor, feed_dict={self.input: data})

        for i in range(10000):
            self.sess.run(output_tensor, feed_dict={self.input: data})
